...
export function usePosKeyboardShortcuts(options) {
  ...
  const shortcutsList = [
    { category: 'Acciones Principales', shortcuts: [
      ...
      { keys: 'Esc', description: 'Cerrar modal / blur input / limpiar busqueda' }
    ]},
    { category: 'Metodos de Pago', shortcuts: [
  ...
  function cyclePaymentMethod(direction = 1) {
    const currentIndex = paymentMethods.findIndex(m => m.value === paymentMethod.value)
  ...
  function handleKeyDown(e) {
    ...
    if (key === 'F10') {
      ...
      if (showInvoiceTicket.value) {
        const printBtn = document.querySelector('[data-shortcut="print-invoice"]')
    ...
    if (e.altKey && !e.ctrlKey && !e.metaKey) {
      ...
      if (paymentMethod.value === 'cash' && quickAmountKeys[key] !== undefined) {
        e.preventDefault()
