...
export const useCustomerTagStore = defineStore("customerTag", () => {
  ...
  async function updateTag(id, data) {
    ...
    try {
      ...
      const message =
        error.response?.data?.message || "Error al actualizar tag";
      toast.error(message);
      throw error;
    } finally {
      loading.value = false;
  ...
  async function assignToCustomers(id, customerIds) {
    loading.value = true;
    try {
      const response = await customerTagService.assignToCustomers(
        id,
        customerIds,
      );
      toast.success(
        response.data.message || "Tag asignado a clientes exitosamente",
      );
      return true;
    } catch (error) {
  ...
  async function removeFromCustomers(id, customerIds) {
    loading.value = true;
    try {
      const response = await customerTagService.removeFromCustomers(
        id,
        customerIds,
      );
      toast.success(
        response.data.message || "Tag removido de clientes exitosamente",
      );
      return true;
    } catch (error) {
