...
export const usePayableStore = defineStore("payable", () => {
  ...
  async function createPayable(data) {
    ...
    try {
      ...
      toast.success(
        response.data.message || "Cuenta por pagar creada exitosamente",
      );
      await fetchPayables();
      return response.data.data;
    } catch (error) {
      const message =
        error.response?.data?.message || "Error al crear cuenta por pagar";
      const errors = error.response?.data?.errors;
  ...
  async function fetchSupplierPendingPayables(supplierId) {
    ...
    try {
      const response =
        await payableService.getSupplierPendingPayables(supplierId);
      return response.data.data;
    } catch (error) {
