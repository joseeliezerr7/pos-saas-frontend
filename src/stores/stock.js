...
export const useStockStore = defineStore("stock", {
  ...
  getters: {
    ...
    outOfStockCount: (state) => {
      return state.lowStockProducts.filter((p) => p.status === "out_of_stock")
        .length;
    },

    lowStockCount: (state) => {
      return state.lowStockProducts.filter((p) => p.status === "low_stock")
        .length;
    },
  ...
  actions: {
    ...
    async fetchMovements(params = {}) {
      ...
      try {
        ...
        this.error =
          error.response?.data?.message || "Error al cargar movimientos";
        throw error;
      } finally {
        this.loading = false;
      }
