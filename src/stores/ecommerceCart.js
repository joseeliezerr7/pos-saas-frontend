...
export const useEcommerceCartStore = defineStore("ecommerceCart", () => {
  ...
  async function addItem(productId, quantity = 1, variantId = null) {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.error?.message || "Error al agregar producto",
      );
      throw error;
    } finally {
      loading.value = false;
  ...
  async function updateItemQuantity(itemId, quantity) {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.error?.message || "Error al actualizar cantidad",
      );
      throw error;
    } finally {
      loading.value = false;
  ...
  async function removeItem(itemId) {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.error?.message || "Error al eliminar producto",
      );
      throw error;
    } finally {
      loading.value = false;
  ...
  async function clearCart() {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.error?.message || "Error al vaciar carrito",
      );
      throw error;
    } finally {
      loading.value = false;
  ...
  async function validateCheckout() {
    try {
      ...
      toast.error(
        error.response?.data?.error?.message || "Error de validacion",
      );
      throw error;
    }
  }
  ...
  async function processCheckout(checkoutData) {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.error?.message || "Error al procesar pedido",
      );
      throw error;
    } finally {
      loading.value = false;
