...
export const useInvoiceStore = defineStore("invoice", () => {
  ...
  async function createInvoice(invoiceData) {
    ...
    try {
      ...
      const message =
        error.response?.data?.error?.message || "Error al generar la factura";
      toast.error(message);
      throw error;
    } finally {
      loading.value = false;
  ...
  async function voidInvoice(id, reason) {
    ...
    try {
      ...
      const message =
        error.response?.data?.error?.message || "Error al anular la factura";
      toast.error(message);
      return false;
    } finally {
      loading.value = false;
  ...
  async function sendEmail(id, email) {
    ...
    try {
      ...
      const message =
        error.response?.data?.error?.message || "Error al enviar la factura";
      toast.error(message);
      return false;
    } finally {
      loading.value = false;
