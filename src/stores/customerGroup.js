...
export const useCustomerGroupStore = defineStore("customerGroup", () => {
  ...
  async function deleteGroup(id) {
    ...
    try {
      ...
      const message =
        error.response?.data?.message || "Error al eliminar grupo";
      toast.error(message);
      return false;
    } finally {
      loading.value = false;
  ...
  async function calculateRFM() {
    ...
    try {
      ...
      toast.success(
        response.data.message || "Análisis RFM calculado exitosamente",
      );
      return true;
    } catch (error) {
      const message =
        error.response?.data?.message || "Error al calcular análisis RFM";
      toast.error(message);
      return false;
    } finally {
      loading.value = false;
  ...
  async function setPrice(id, data) {
    ...
    try {
      ...
      const message =
        error.response?.data?.message || "Error al configurar precio";
      toast.error(message);
      throw error;
    } finally {
      loading.value = false;
  ...
  async function assignCustomers(id, customerIds) {
    loading.value = true;
    try {
      const response = await customerGroupService.assignCustomers(
        id,
        customerIds,
      );
      ...
      const message =
        error.response?.data?.message || "Error al asignar clientes";
      toast.error(message);
      return false;
    } finally {
      loading.value = false;
