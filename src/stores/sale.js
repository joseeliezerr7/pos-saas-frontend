...
export const useSaleStore = defineStore("sale", () => {
  ...
  async function createSale(saleData) {
    ...
    try {
      ...
      const message =
        error.response?.data?.error?.message || "Error al crear la venta";
      toast.error(message);
      throw error;
    } finally {
      loading.value = false;
  ...
  function addToCart(product, quantity = 1) {
    ...
    if (existingItem) {
      ...
      const specialPrice = groupPrices.value.find(
        (gp) => gp.product_id === product.id,
      );
      if (specialPrice) {
  ...
  async function setCustomer(customerData) {
    ...
    if (customerData && customerData.customer_group_id) {
      try {
        const response = await customerGroupService.getPrices(
