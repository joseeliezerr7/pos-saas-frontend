...
export const usePromotionStore = defineStore("promotion", () => {
  ...
  async function getApplicablePromotions(
    cartItems,
    branchId,
    customerId = null,
  ) {
    try {
      const response = await promotionService.getApplicablePromotions(
        cartItems,
        branchId,
        customerId,
      );
  ...
  async function applyPromotion(promotionId, cartItems) {
    try {
      ...
      const message =
        error.response?.data?.message || "Error al aplicar promoción";
      toast.error(message);
      throw error;
    }
  }
