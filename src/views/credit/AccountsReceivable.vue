<template>
  <div class="accounts-receivable-page">
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
      ...
      <table v-else class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr
            v-for="creditSale in creditSales"
            :key="creditSale.id"
            class="hover:bg-gray-50"
          >
            <td class="px-6 py-4 whitespace-nowrap">
              <span class="text-sm font-medium text-gray-900">{{
                creditSale.sale?.sale_number
              }}</span>
            </td>
            <td class="px-6 py-4 whitespace-nowrap">
            ...
            <td class="px-6 py-4 whitespace-nowrap">
              ...
              <div
                v-if="creditSale.days_overdue > 0"
              ...
              <div
                v-else-if="
                  daysUntilDue(creditSale.due_date) <= 7 &&
                  daysUntilDue(creditSale.due_date) > 0
                "
            ...
            <td class="px-6 py-4 whitespace-nowrap text-right">
              <span
                ...
                :class="
                  creditSale.balance_due > 0 ? 'text-red-600' : 'text-green-600'
                "
              >
    ...
    <div
      v-if="showDetailsModal && selectedCreditSale"
          ...
          <div class="flex justify-between items-start mb-6">
            <div>
              <h2 class="text-2xl font-bold text-gray-800">
                Detalles de Venta al Crédito
              </h2>
              <p class="text-gray-600">
                {{ selectedCreditSale.sale?.sale_number }}
              </p>
            </div>
            <button
          ...
          <div class="grid grid-cols-2 gap-4 mb-6">
            <div>
              ...
              <p class="text-gray-900 font-medium">
                {{ selectedCreditSale.customer?.name }}
              </p>
            </div>
            <div>
              ...
              <p class="text-gray-900">
                {{ formatDate(selectedCreditSale.sale?.sold_at) }}
              </p>
            </div>
            <div>
              <label class="text-sm font-medium text-gray-500"
              ...
              >
              <p class="text-gray-900">
                {{ formatDate(selectedCreditSale.due_date) }}
              </p>
            </div>
            <div>
          ...
          <div class="grid grid-cols-3 gap-4 mb-6">
            <div class="bg-blue-50 p-4 rounded-lg">
              ...
              <div class="text-xl font-semibold text-blue-600">
                L {{ formatCurrency(selectedCreditSale.original_amount) }}
              </div>
            </div>
            <div class="bg-green-50 p-4 rounded-lg">
              <div class="text-sm text-gray-600 mb-1">Monto Pagado</div>
              <div class="text-xl font-semibold text-green-600">
                L {{ formatCurrency(selectedCreditSale.amount_paid) }}
              </div>
            </div>
            <div class="bg-red-50 p-4 rounded-lg">
              <div class="text-sm text-gray-600 mb-1">Saldo Pendiente</div>
              <div class="text-xl font-semibold text-red-600">
                L {{ formatCurrency(selectedCreditSale.balance_due) }}
              </div>
            </div>
          </div>
          ...
          <div v-if="selectedCreditSale.allocations?.length > 0">
            ...
            <div class="overflow-x-auto">
              <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                  <tr
                    v-for="allocation in selectedCreditSale.allocations"
                    :key="allocation.id"
                  >
                    <td class="px-4 py-2 text-sm text-gray-900">
                      {{
                        formatDate(allocation.customer_payment?.payment_date)
                      }}
                    </td>
                    <td class="px-4 py-2 text-sm text-gray-900">
                      {{ allocation.customer_payment?.payment_number }}
                    </td>
                    <td
...
const stats = computed(() => {
  ...
  return {
    current_count: sales.filter(
      (s) => s.status === "pending" && s.days_overdue === 0,
    ).length,
    current_amount: sales
