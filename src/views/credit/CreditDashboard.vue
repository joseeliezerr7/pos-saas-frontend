    ...
    <template v-else-if="creditStore.dashboardStats">
          ...
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-2xl font-bold text-gray-900">
                L
                {{ formatNumber(creditStore.dashboardStats.total_receivable) }}
              </p>
              <p class="text-xs text-gray-500 mt-1">
                {{ creditStore.dashboardStats.total_credit_sales }} ventas
              </p>
            </div>
          ...
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-2xl font-bold text-red-600">
                L {{ formatNumber(creditStore.dashboardStats.overdue_amount) }}
              </p>
              <p class="text-xs text-gray-500 mt-1">
                {{ creditStore.dashboardStats.overdue_count }} facturas
              </p>
            </div>
          ...
          <div class="flex items-center justify-between">
            <div class="flex-1">
              ...
              <div class="mt-1">
                <p
                  v-if="
                    !creditStore.dashboardStats.customers_over_limit?.count ||
                    creditStore.dashboardStats.customers_over_limit.count === 0
                  "
          ...
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-2xl font-bold text-green-600">
                L
                {{
                  formatNumber(creditStore.dashboardStats.payments_this_month)
                }}
              </p>
              <p class="text-xs text-gray-500 mt-1">
                {{ creditStore.dashboardStats.payment_count_this_month }} pagos
              </p>
            </div>
      ...
      <div class="card">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div
            v-if="creditStore.dashboardStats.aging"
          ...
          >
            ...
            <p class="text-2xl font-bold text-green-900 mt-1">
              ...
              {{
                formatNumber(
                  creditStore.dashboardStats.aging.current?.amount || 0,
                )
              }}
            </p>
            <p class="text-xs text-green-600 mt-1">
              {{ creditStore.dashboardStats.aging.current?.count || 0 }}
              facturas
            </p>
          </div>
          <div
          ...
          >
            ...
            <p class="text-2xl font-bold text-yellow-900 mt-1">
              ...
              {{
                formatNumber(
                  creditStore.dashboardStats.aging["1_30_days"]?.amount || 0,
                )
              }}
            </p>
            <p class="text-xs text-yellow-600 mt-1">
              {{ creditStore.dashboardStats.aging["1_30_days"]?.count || 0 }}
              facturas
            </p>
          </div>
          <div
          ...
          >
            ...
            <p class="text-2xl font-bold text-orange-900 mt-1">
              ...
              {{
                formatNumber(
                  creditStore.dashboardStats.aging["31_60_days"]?.amount || 0,
                )
              }}
            </p>
            <p class="text-xs text-orange-600 mt-1">
              {{ creditStore.dashboardStats.aging["31_60_days"]?.count || 0 }}
              facturas
            </p>
          </div>
          <div
          ...
          >
            ...
            <p class="text-2xl font-bold text-red-900 mt-1">
              ...
              {{
                formatNumber(
                  creditStore.dashboardStats.aging["60_plus_days"]?.amount || 0,
                )
              }}
            </p>
            <p class="text-xs text-red-600 mt-1">
              {{ creditStore.dashboardStats.aging["60_plus_days"]?.count || 0 }}
              facturas
            </p>
          </div>
        </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="card">
          <h2 class="text-lg font-semibold mb-4">
            Principales Clientes con Saldo
          </h2>
          <div
            v-if="
              !creditStore.dashboardStats.top_customers ||
              creditStore.dashboardStats.top_customers.length === 0
            "
          ...
          <div v-else class="space-y-3">
            <div
              v-for="(
                customer, index
              ) in creditStore.dashboardStats.top_customers.slice(0, 5)"
              :key="customer.id"
              ...
              <div class="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p class="text-sm text-gray-500">
                    {{ customer.pending_sales }} facturas pendientes
                  </p>
                </div>
              </div>
              <div class="text-right">
                <p class="font-semibold text-gray-900">
                  L {{ formatNumber(customer.balance) }}
                </p>
                <router-link
                  :to="`/credit/customer-statement/${customer.id}`"
        ...
        <div class="card">
          ...
          <div
            v-if="
              !creditStore.dashboardStats.recent_payments ||
              creditStore.dashboardStats.recent_payments.length === 0
            "
            class="text-center py-8 text-gray-500"
          >
          ...
          <div v-else class="space-y-3">
            <div
              v-for="payment in creditStore.dashboardStats.recent_payments.slice(
                0,
                5,
              )"
              :key="payment.id"
    ...
    >
      ...
      >
        ...
        <div class="p-6 overflow-y-auto max-h-[60vh]">
          <div
            v-if="
              creditStore.dashboardStats?.customers_over_limit?.customers
                ?.length > 0
            "
            class="space-y-4"
          >
            <div
              v-for="customer in creditStore.dashboardStats.customers_over_limit
                .customers"
              :key="customer.id"
                  ...
                  <div class="grid grid-cols-2 gap-4 mt-3">
                    <div>
                      ...
                      <p class="text-sm font-medium text-gray-900">
                        L {{ formatNumber(customer.credit_limit) }}
                      </p>
                    </div>
                    <div>
                      <p class="text-xs text-gray-500">Balance Actual</p>
                      <p class="text-sm font-semibold text-orange-600">
                        L {{ formatNumber(customer.current_balance) }}
                      </p>
                    </div>
                    <div>
          ...
          <div v-else class="text-center py-12">
            ...
            <p class="text-gray-600 font-medium">
              Todos los clientes dentro del límite de crédito
            </p>
