<template>
  <div>
    ...
    <!-- Loading State -->
    <loading-spinner
      v-if="settingsStore.loading && cais.length === 0"
      text="Cargando CAIs..."
    />
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="cai in cais" :key="cai.id" class="hover:bg-gray-50">
              <td class="px-6 py-4">
                ...
                <div v-if="cai.branch" class="text-xs text-gray-500">
                  {{ cai.branch.name }}
                </div>
              </td>
              <td class="px-6 py-4 text-sm text-gray-900">
                {{ getDocumentTypeLabel(cai.document_type) }}
              </td>
              ...
              <td class="px-6 py-4">
                <div class="text-sm text-gray-900">
                  {{ cai.used_documents }} / {{ cai.total_documents }}
                </div>
                <div class="w-full bg-gray-200 rounded-full h-2 mt-1">
                  <div
                    class="bg-primary-600 h-2 rounded-full"
                    :style="{
                      width: `${(cai.used_documents / cai.total_documents) * 100}%`,
                    }"
                  ></div>
                </div>
              </td>
              ...
              <td class="px-6 py-4">
                <span
                  :class="getStatusClass(cai.status)"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            >
              ...
              <option value="FACTURA_EXPORTACION">
...
onMounted(async () => {
  await Promise.all([loadCAIs(), loadBranches()]);
});
...
async function saveCAI() {
  try {
    if (selectedCAI.value) {
      await settingsStore.updateCAI(selectedCAI.value.id, {
    ...
    toast.error(
      selectedCAI.value ? "Error al actualizar CAI" : "Error al crear CAI",
    );
  }
}
