    ...
    <div v-else class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="lg:col-span-1">
        <div class="bg-white rounded-lg shadow p-6">
          ...
          <div class="flex flex-col items-center">
            <!-- Logo Preview -->
            <div
              class="w-48 h-48 border-2 border-dashed border-gray-300 rounded-lg flex items-center justify-center mb-4 bg-gray-50"
            >
              <img
                v-if="companyLogoUrl"
                :src="companyLogoUrl"
            ...
            <div class="flex space-x-2">
              <label
                class="px-4 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700 cursor-pointer text-sm"
              >
                <input
                  type="file"
      ...
      <div class="lg:col-span-2">
        ...
        <div class="bg-white rounded-lg shadow p-6 mt-6">
          ...
          <div class="space-y-4">
            ...
            >
              <input
                type="checkbox"
                v-model="form.settings.enable_order_numbers"
                class="w-5 h-5 rounded border-blue-300 text-blue-600 focus:ring-blue-500"
              />
              <div>
                <span class="text-sm font-semibold text-blue-800"
                  >Habilitar No. de Orden</span
                >
                <span class="text-xs text-blue-600 block"
                  >Muestra la opción de asignar número de orden en el POS (útil
        ...
        <div class="bg-white rounded-lg shadow p-6 mt-6">
          ...
          <div class="space-y-4">
            <label
              class="flex items-center gap-3 p-3 bg-orange-50 border border-orange-200 rounded-lg cursor-pointer hover:bg-orange-100 transition-colors"
            >
              <input
                type="checkbox"
                v-model="form.settings.enable_kds"
              ...
              <div>
                <span class="text-sm font-semibold text-orange-800"
                  >Habilitar Pantalla de Cocina</span
                >
                <span class="text-xs text-orange-600 block"
                  >Activa el sistema KDS para que la cocina vea y gestione los
                  pedidos en tiempo real</span
                >
              </div>
            </label>

            <div
              v-if="form.settings.enable_kds"
              class="space-y-4 pl-4 border-l-2 border-orange-200 ml-2"
            >
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2"
                  >Tiempo de alerta (minutos)</label
                >
                <input
                ...
                <p class="text-xs text-gray-500 mt-1">
                  Los pedidos que superen este tiempo se resaltaran en rojo
                </p>
              </div>

              <label
                class="flex items-center gap-3 p-3 bg-gray-50 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-100 transition-colors"
              >
                <input
                  type="checkbox"
                  v-model="form.settings.kds_sound"
                  class="w-5 h-5 rounded border-gray-300 text-orange-600 focus:ring-orange-500"
                />
                <div>
                  <span class="text-sm font-semibold text-gray-800"
                    >Sonido de notificacion</span
                  >
                  <span class="text-xs text-gray-600 block"
                    >Reproduce un sonido cuando llega un nuevo pedido a la
                    cocina</span
                  >
                </div>
              </label>
