<template>
  <div>
      ...
      <div class="flex items-start">
        ...
        <div>
          <h3 class="text-lg font-semibold text-red-800">
            Atención: Suscripción Inactiva
          </h3>
          <p class="text-red-700 mt-1">{{ warningMessage }}</p>
          <p class="text-red-600 text-sm mt-2">
            Para continuar usando el sistema, por favor renueva tu suscripción o
    ...
    <!-- Loading State -->
    <loading-spinner
      v-if="settingsStore.loading && !company"
    ...
    <div v-else-if="company">
        ...
        <div class="mb-6">
          ...
          <div class="space-y-4">
            ...
            <div v-if="planLimits.max_users">
              ...
              <div
                v-if="planLimits.max_users !== -1"
                class="w-full bg-gray-200 rounded-full h-2"
              >
                <div
                  class="bg-primary-600 h-2 rounded-full"
                  :style="{
                    width: `${getUsagePercentage(currentUsage.users, planLimits.max_users)}%`,
                  }"
                ></div>
              </div>
            </div>
            ...
            <div v-if="planLimits.max_products">
              ...
              <div
                v-if="planLimits.max_products !== -1"
                class="w-full bg-gray-200 rounded-full h-2"
              >
                <div
                  class="bg-primary-600 h-2 rounded-full"
                  :style="{
                    width: `${getUsagePercentage(currentUsage.products, planLimits.max_products)}%`,
                  }"
                ></div>
              </div>
            </div>
            ...
            <div v-if="planLimits.max_branches">
              ...
              <div
                v-if="planLimits.max_branches !== -1"
                class="w-full bg-gray-200 rounded-full h-2"
              >
                <div
                  class="bg-primary-600 h-2 rounded-full"
                  :style="{
                    width: `${getUsagePercentage(currentUsage.branches, planLimits.max_branches)}%`,
                  }"
                ></div>
              </div>
            </div>
            ...
            <div v-if="planLimits.max_monthly_sales">
              <div class="flex justify-between text-sm mb-2">
                ...
                <span class="text-gray-600">
                  {{ currentUsage.monthly_sales || 0 }} /
                  {{
                    planLimits.max_monthly_sales === -1
                      ? "Ilimitado"
                      : planLimits.max_monthly_sales
                  }}
                </span>
              </div>
              <div
                v-if="planLimits.max_monthly_sales !== -1"
                class="w-full bg-gray-200 rounded-full h-2"
              >
                <div
                  class="bg-primary-600 h-2 rounded-full"
                  :style="{
                    width: `${getUsagePercentage(currentUsage.monthly_sales, planLimits.max_monthly_sales)}%`,
                  }"
                ></div>
              </div>
            </div>
      ...
      >
        ...
        <div class="space-y-3">
          ...
          <div
            v-if="subscription.last_payment_date"
          ...
          <div
            v-if="subscription.last_payment_amount"
            class="flex justify-between text-sm"
          >
            <span class="text-gray-600">Monto del Último Pago</span>
            <span class="font-medium text-gray-900">{{
              formatCurrency(subscription.last_payment_amount)
            }}</span>
          </div>
        </div>
      </div>
    ...
    <div v-else class="bg-white rounded-lg shadow p-8 text-center">
      ...
      <p class="text-lg font-medium text-gray-900">
        No hay información de suscripción
      </p>
...
onMounted(async () => {
  ...
  if (route.query.warning) {
    ...
    warningMessage.value =
      route.query.message || "Tu suscripción requiere atención.";
...
const planDescription = computed(
  () => plan.value?.description || "Plan básico sin costo",
);
...
const planFeatures = computed(() => {
  ...
  if (planFeats.max_users) {
    features.push({
      name: "Usuarios",
      description:
        planFeats.max_users === -1
          ? "Usuarios ilimitados"
          : `Hasta ${planFeats.max_users} usuarios`,
    });
  }
  ...
  if (planFeats.max_products) {
    features.push({
      name: "Productos",
      description:
        planFeats.max_products === -1
          ? "Productos ilimitados"
          : `Hasta ${planFeats.max_products} productos`,
    });
  }
  ...
  if (planFeats.max_branches) {
    features.push({
      name: "Sucursales",
      description:
        planFeats.max_branches === -1
          ? "Sucursales ilimitadas"
          : `Hasta ${planFeats.max_branches} sucursales`,
    });
  }
  ...
  if (planFeats.max_monthly_sales) {
    features.push({
      ...
      description:
        planFeats.max_monthly_sales === -1
          ? "Ventas ilimitadas"
          : `Hasta ${planFeats.max_monthly_sales} ventas al mes`,
    });
  }
  ...
  if (planFeats.pos_terminals) {
    features.push({
      name: "Terminales POS",
      description:
        planFeats.pos_terminals === -1
          ? "Terminales ilimitadas"
          : `Hasta ${planFeats.pos_terminals} terminales`,
    });
  }
