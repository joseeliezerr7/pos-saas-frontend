        ...
        <div class="flex items-center">
          ...
          <div>
            ...
            <p class="text-xs text-red-600 mt-1">
              Anulada el {{ formatDate(invoice.voided_at) }}
            </p>
          </div>
        </div>
      </div>
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
          ...
          <dl class="space-y-3">
            ...
            <div>
              ...
              <dd class="text-base text-gray-900">
                {{ formatDate(invoice.cai_expiration) }}
              </dd>
            </div>
            <div>
        ...
        <div class="max-w-md ml-auto space-y-3">
          <div class="flex justify-between text-base">
            ...
            <span class="font-medium text-gray-900"
              >L {{ formatMoney(invoice.subtotal) }}</span
            >
          </div>
          <div class="flex justify-between text-base">
            <span class="text-gray-600">Descuento:</span>
            <span class="font-medium text-gray-900"
              >L {{ formatMoney(invoice.discount) }}</span
            >
          </div>
          <div class="flex justify-between text-base">
            <span class="text-gray-600">Subtotal Gravado:</span>
            <span class="font-medium text-gray-900"
              >L {{ formatMoney(invoice.taxable_subtotal) }}</span
            >
          </div>
          <div class="flex justify-between text-base">
            <span class="text-gray-600">Subtotal Exento:</span>
            <span class="font-medium text-gray-900"
              >L {{ formatMoney(invoice.exempt_subtotal) }}</span
            >
          </div>
          <div class="flex justify-between text-base">
            <span class="text-gray-600">ISV (15%):</span>
            <span class="font-medium text-gray-900"
              >L {{ formatMoney(invoice.tax) }}</span
            >
          </div>
          <div
          ...
          >
            ...
            <span class="text-gray-900"
              >L {{ formatMoney(invoice.total) }}</span
            >
          </div>
        </div>
      </div>
        ...
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-sm text-blue-700">
              ...
              {{ formatDate(invoice.sale.created_at) }}
            </p>
          </div>
          <button
...
async function confirmVoid() {
  ...
  const success = await invoiceStore.voidInvoice(
    invoice.value.id,
    voidReason.value,
  );
  if (success) {
