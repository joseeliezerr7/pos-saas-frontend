...
const averageOrder = computed(() => {
  if (!customer.value?.orders_count || customer.value.orders_count === 0)
    return 0;
...
const updateCustomerType = async (newType) => {
  try {
    await ecommerceService.updateCustomerType(customer.value.id, {
      type: newType,
    });
...
const getOrderStatusClass = (status) => {
  const classes = {
    ...
    shipped:
      "px-2 py-1 text-xs font-medium rounded-full bg-indigo-100 text-indigo-800",
    delivered:
