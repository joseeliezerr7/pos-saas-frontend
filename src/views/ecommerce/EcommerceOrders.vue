              ...
              <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                ...
                <div class="mt-2">
                  <p class="text-sm text-gray-500">
                    ¿Estás seguro de convertir el pedido <strong>#{{ orderToConvert?.order_number }}</strong> a una venta en el POS?
                  </p>
                </div>
              </div>
            </div>
    ...
    <div class="bg-white rounded-lg shadow-sm p-4">
      <div class="flex flex-col md:flex-row gap-4">
        ...
        <div class="flex gap-2 flex-wrap">
          <select v-model="statusFilter" @change="loadOrders" class="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
          ...
          <select v-model="dateFilter" @change="loadOrders" class="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
        ...
        <table class="w-full">
          ...
          <tbody class="divide-y divide-gray-200">
            <tr v-for="order in orders" :key="order.id" class="hover:bg-gray-50">
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div>
                  <p class="text-sm font-medium text-gray-900">{{ order.customer?.name || order.customer_name || 'Cliente Anónimo' }}</p>
...
const { orders, orderStats, ordersPagination, loading } = storeToRefs(ecommerceStore)
...
const updatePaymentStatus = async (orderId, newStatus) => {
  try {
    await ecommerceService.updatePaymentStatus(orderId, { payment_status: newStatus })
    toast.success('Estado de pago actualizado')
    await loadOrders(pagination.value.currentPage)
  } catch (error) {
    console.error('Error updating payment status:', error)
    toast.error(error.response?.data?.error?.message || 'Error al actualizar estado de pago')
    await loadOrders(pagination.value.currentPage)
  }
}
