<template>
  <div class="rounded-lg p-3 border-2 transition-all" :class="cardClasses">
    ...
    <div class="space-y-1 mb-3">
      ...
      >
        <span class="font-bold text-lg leading-tight min-w-[2rem] text-right"
          >{{ Math.round(detail.quantity) }}x</span
        >
        <div class="flex-1 min-w-0">
          ...
          <p
            ...
            class="text-xs text-gray-400 mt-0.5 leading-snug"
          >
            {{ detail.product.description }}
          </p>
          <p v-if="detail.notes" class="text-xs text-yellow-400 mt-0.5 italic">
            {{ detail.notes }}
          </p>
        </div>
      </div>
    ...
    <div
      ...
      class="bg-yellow-900/30 border border-yellow-700/50 rounded px-2 py-1 mb-3"
    >
