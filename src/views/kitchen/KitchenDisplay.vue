<template>
  <div class="min-h-screen bg-gray-900 text-white flex flex-col">
    <!-- Header -->
    <header
      class="bg-gray-800 px-3 sm:px-4 py-2 sm:py-3 flex items-center justify-between flex-shrink-0 border-b border-gray-700"
    >
      <div class="flex items-center gap-2 sm:gap-4 min-w-0">
        <h1
          class="text-base sm:text-xl font-bold flex items-center gap-1.5 sm:gap-2 whitespace-nowrap"
        >
          <svg
            class="w-5 h-5 sm:w-7 sm:h-7 text-orange-400 flex-shrink-0"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
        ...
        <span
          v-if="stats"
          class="hidden md:inline text-sm text-gray-400 truncate"
        >
          Hoy: {{ stats.total_today }} pedidos | Promedio:
          {{ formatTime(stats.avg_prep_time_seconds) }}
        </span>
      </div>
    ...
    <!-- Stats Bar -->
    <div
      class="bg-gray-800/50 px-3 sm:px-4 py-2 border-b border-gray-700/50 flex-shrink-0"
    >
      <div
        class="flex flex-wrap items-center gap-x-4 sm:gap-x-6 gap-y-1 text-xs sm:text-sm"
      >
        <div class="flex items-center gap-1.5">
          <span
            class="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full bg-yellow-500"
          ></span>
        ...
        <div class="flex items-center gap-1.5">
          <span
            class="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full bg-blue-500"
          ></span>
        ...
        <div class="flex items-center gap-1.5">
          <span
            class="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full bg-green-500"
          ></span>
        ...
        <div class="hidden sm:flex items-center gap-1.5">
          <span
            class="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full bg-gray-500"
          ></span>
    ...
    <div class="flex-1 flex overflow-hidden p-2 sm:p-3 gap-2 sm:gap-3">
      ...
      >
        <div
          class="bg-yellow-600 rounded-t-lg px-3 sm:px-4 py-2 flex items-center justify-between flex-shrink-0"
        >
          <h2 class="text-sm sm:text-lg font-bold">PENDIENTES</h2>
          <span
            class="bg-yellow-800 px-2 py-0.5 rounded-full text-xs sm:text-sm font-bold"
            >{{ pendingOrders.length }}</span
          >
        </div>
        <div
          class="flex-1 overflow-y-auto bg-gray-800/30 rounded-b-lg p-2 space-y-2"
        >
          <OrderCard
          ...
          <div
            ...
            class="flex items-center justify-center h-32 text-gray-600 text-sm"
          >
      ...
      >
        <div
          class="bg-blue-600 rounded-t-lg px-3 sm:px-4 py-2 flex items-center justify-between flex-shrink-0"
        >
          <h2 class="text-sm sm:text-lg font-bold">PREPARANDO</h2>
          <span
            class="bg-blue-800 px-2 py-0.5 rounded-full text-xs sm:text-sm font-bold"
      ...
      >
        <div
          class="bg-green-600 rounded-t-lg px-3 sm:px-4 py-2 flex items-center justify-between flex-shrink-0"
        >
          <h2 class="text-sm sm:text-lg font-bold">LISTOS</h2>
          <span
            class="bg-green-800 px-2 py-0.5 rounded-full text-xs sm:text-sm font-bold"
    ...
    <div
      v-if="showHistory"
      class="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black bg-opacity-70"
    >
      <div
        class="bg-gray-800 rounded-t-xl sm:rounded-lg shadow-2xl w-full sm:max-w-2xl sm:mx-4 max-h-[85vh] sm:max-h-[80vh] flex flex-col"
      >
        <div
          class="flex items-center justify-between p-3 sm:p-4 border-b border-gray-700"
        >
        ...
        <div class="flex-1 overflow-y-auto p-3 sm:p-4 space-y-2">
          ...
          >
            <div class="min-w-0 flex-1">
              ...
              <span
                v-if="order.customer"
                class="ml-2 text-gray-400 text-xs sm:text-sm truncate"
                >{{ order.customer.name }}</span
              >
              <div class="text-xs text-gray-500 mt-1">
                {{ order.details?.length || 0 }} items | L
                {{ parseFloat(order.total).toFixed(2) }}
              </div>
            </div>
            <div class="text-right text-sm flex-shrink-0 ml-2">
              <span class="text-green-400 font-medium text-xs sm:text-sm"
                >Entregado</span
              >
              <div
                v-if="order.kitchen_started_at && order.kitchen_completed_at"
          ...
          <div
            v-if="kitchenStore.history.length === 0"
...
const alertMinutes = computed(
  () => companySettings.value.kds_alert_minutes || 15,
);
...
function updateClock() {
  const now = new Date();
  currentTime.value = now.toLocaleTimeString("es-HN", {
    hour: "2-digit",
    minute: "2-digit",
    second: "2-digit",
  });
...
function playBeep(frequency, durationMs, volume) {
  ...
  gain.gain.exponentialRampToValueAtTime(
    0.01,
    kitchenAudioCtx.currentTime + durationMs / 1000,
  );
