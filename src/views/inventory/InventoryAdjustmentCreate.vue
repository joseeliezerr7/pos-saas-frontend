    ...
    <div class="flex justify-between items-center">
      <div>
        ...
        <p class="text-gray-600 mt-1">
          Registra diferencias entre inventario físico y sistema
        </p>
      </div>
      <router-link
    ...
    <form @submit.prevent="handleSubmit" class="space-y-6">
      ...
      <div class="bg-white rounded-lg shadow-sm p-6">
        ...
        <div
          ...
          class="mt-4 border rounded-lg overflow-hidden max-h-64 overflow-y-auto"
        >
          <div
...
async function addProduct(product) {
  ...
  try {
    ...
    if (response.data.success) {
      const stockItem = response.data.data.find(
  ...
  try {
    ...
    toast.error(
      error.response?.data?.error?.message ||
        "Error al crear el ajuste de inventario",
    );
  } finally {
    loading.value = false;
  }
