<template>
  <div class="p-6">
    ...
    <div v-else-if="adjustment" class="space-y-6">
        ...
        <div class="flex items-center">
          ...
          <div>
            ...
            <p class="text-sm text-green-700">
              Aprobado por {{ adjustment.approver_name }} el
              {{ formatDateTime(adjustment.approved_at) }}
            </p>
          </div>
        </div>
      </div>
      ...
      <div
        v-else-if="adjustment.status === 'rejected'"
        ...
        <div class="flex items-center">
          ...
          <div>
            ...
            <p class="text-sm text-red-700">
              Rechazado por {{ adjustment.approver_name }} el
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
          ...
          <dl class="space-y-3">
            <div>
              ...
              <dd class="text-base text-gray-900">
                {{ adjustment.items_count }} productos
              </dd>
            </div>
            <div>
              <dt class="text-sm font-medium text-gray-500">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="item in adjustment.details" :key="item.id">
                <td class="px-6 py-4">
                  ...
                  <div v-if="item.variant_name" class="text-xs text-gray-500">
...
function confirmApprove() {
  if (
    confirm(
      `¿Aprobar el ajuste ${adjustment.value.adjustment_number}? Esto actualizará el inventario.`,
    )
  ) {
    approveAdjustment();
  }
...
async function approveAdjustment() {
  ...
  try {
    ...
    toast.error(
      error.response?.data?.error?.message || "Error al aprobar el ajuste",
    );
  } finally {
    loading.value = false;
  }
...
async function rejectAdjustment() {
  ...
  try {
    ...
    toast.error(
      error.response?.data?.error?.message || "Error al rechazar el ajuste",
    );
  } finally {
    loading.value = false;
  }
...
async function deleteAdjustment() {
  ...
  try {
    ...
    toast.error(
      error.response?.data?.error?.message || "Error al eliminar el ajuste",
    );
  } finally {
    loading.value = false;
  }
