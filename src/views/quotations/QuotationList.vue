<template>
  <div class="quotations p-6">
    <div class="mb-6 flex justify-between items-center">
      <div>
        ...
        <p class="text-gray-600 mt-2">
          Gestione las cotizaciones y conviértalas en ventas
        </p>
      </div>
      <router-link
      ...
      <div class="p-6">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="quotation in quotations" :key="quotation.id">
                ...
                <td class="px-6 py-4 text-sm text-gray-900">
                  ...
                  <div v-if="quotation.customer_rtn" class="text-gray-500">
                    RTN: {{ quotation.customer_rtn }}
                  </div>
                </td>
                <td
                  class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-right text-gray-900"
                >
        ...
        >
          <div class="text-sm text-gray-700">
            Mostrando {{ quotations.length }} de
    ...
    <div
      v-if="showConvertToSaleModal"
      class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50"
    >
      <div class="bg-white rounded-lg max-w-md w-full p-6">
        <h2 class="text-xl font-bold mb-4">Convertir a Venta</h2>
        <p class="text-gray-600 mb-4">
          ¿Desea convertir la cotización
          <strong>{{ selectedQuotation?.quotation_number }}</strong> en una
          venta?
        </p>
    ...
    >
      <div class="bg-white rounded-lg max-w-md w-full p-6">
        ...
        <p class="text-gray-600 mb-6">
          ...
          <strong>{{ selectedQuotation?.quotation_number }}</strong
...
async function confirmConvert() {
  ...
  try {
    const sale = await quotationStore.convertToSale(
