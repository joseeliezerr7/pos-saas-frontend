<template>
  <div class="space-y-6">
    ...
    <div class="card">
            ...
            >
              <td class="px-6 py-4">
                <div>
                  ...
                  <span
                    v-if="role.is_system"
                    class="inline-flex mt-1 items-center px-2 py-0.5 rounded text-xs font-medium bg-purple-100 text-purple-800"
                  >
                    Sistema
                  </span>
                </div>
      ...
      <!-- Pagination -->
      <div
        v-if="roleStore.pagination.total > roleStore.pagination.per_page"
        class="mt-4 flex justify-between items-center"
      >
        <div class="text-sm text-gray-500">
          Mostrando {{ roleStore.roles.length }} de
          {{ roleStore.pagination.total }} roles
        </div>
...
async function openEditModal(role) {
  ...
  if (fullRole) {
    form.value = {
      ...
      permission_ids: fullRole.permissions
        ? fullRole.permissions.map((p) => p.id)
        : [],
    };
  }
}
