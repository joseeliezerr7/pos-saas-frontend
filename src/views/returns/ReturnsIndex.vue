<template>
  <div>
            ...
            >
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                ...
                <div class="text-xs text-gray-500">
                  {{ formatTime(returnItem.returned_at) }}
                </div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-sm text-gray-900">
              ...
              <td class="px-6 py-4">
                <div class="text-sm text-gray-900">
                  {{ returnItem.customer_name || "Cliente General" }}
                </div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                ...
                <div class="text-xs text-gray-500">
                  {{ returnItem.details?.length || 0 }} items
                </div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <span
                  :class="getReturnTypeClass(returnItem.return_type)"
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm">
                <div class="flex space-x-2">
                  ...
                  <button
                    v-if="
                      returnItem.status === 'pending' && can('create_sales')
                    "
      ...
      <div v-if="selectedReturn" class="space-y-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 gap-4 pb-4 border-b">
          <div>
            ...
            <p class="font-medium">
              {{ formatDate(selectedReturn.returned_at) }}
              {{ formatTime(selectedReturn.returned_at) }}
            </p>
          </div>
          <div>
            <p class="text-sm text-gray-500">Cliente</p>
            <p class="font-medium">
              {{ selectedReturn.customer_name || "Cliente General" }}
            </p>
          </div>
          <div>
            <p class="text-sm text-gray-500">Venta Referencia</p>
            <p class="font-medium">
              {{ selectedReturn.sale?.sale_number || "N/A" }}
            </p>
          </div>
          <div>
            <p class="text-sm text-gray-500">Método de Reembolso</p>
            <p class="font-medium">
              {{ getRefundMethodLabel(selectedReturn.refund_method) }}
            </p>
          </div>
          <div>
            ...
            <tbody class="divide-y divide-gray-200">
              <tr v-for="item in selectedReturn.details" :key="item.id">
                <td class="px-4 py-3">
                  ...
                  <div v-if="item.reason" class="text-xs text-gray-600 mt-1">
                    Razón: {{ item.reason }}
                  </div>
                </td>
          ...
          <div class="flex justify-end">
            <div class="w-64 space-y-2">
              ...
              <div class="flex justify-between">
                ...
                <span class="font-medium">{{
                  formatCurrency(selectedReturn.discount)
                }}</span>
              </div>
              <div class="flex justify-between">
    ...
    >
      <div class="space-y-4">
        <p class="text-gray-700">
          ¿Estás seguro de que deseas completar la devolución
          <strong>{{ returnToComplete?.return_number }}</strong
          >?
        </p>
        <p class="text-sm text-gray-600">
          Se procesará el reembolso de
          <strong>{{
            formatCurrency(returnToComplete?.refund_amount || 0)
          }}</strong>
    ...
    >
      <div class="space-y-4">
        <p class="text-gray-700">
          ¿Estás seguro de que deseas cancelar la devolución
          <strong>{{ returnToCancel?.return_number }}</strong
          >?
        </p>
        <p class="text-sm text-gray-600">
