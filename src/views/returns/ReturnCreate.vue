<template>
  <div class="max-w-6xl mx-auto">
        ...
        >
          <div class="flex items-start space-x-4">
            ...
            <div class="flex-1">
              ...
              <div
                v-if="returnItems[index].selected"
                class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-3 pt-3 border-t"
              >
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-1"
    ...
    <div
      v-if="selectedSale && hasSelectedItems"
          ...
          <div class="bg-gray-50 rounded-lg p-4 sticky top-4">
            ...
            <div v-if="returnData.refund_method" class="mt-4 pt-4 border-t">
              ...
              <p class="text-xs text-gray-500 mt-1">
                Máximo: {{ formatCurrency(returnTotals.total) }}
              </p>
            </div>
...
async function processReturn() {
  ...
  const hasInvalidItems = returnItems.value.some(
    (item) => item.selected && !item.reason,
  );

  if (hasInvalidItems) {
    toast.error(
      "Por favor especifica el motivo para todos los productos seleccionados",
    );
    return;
  }
