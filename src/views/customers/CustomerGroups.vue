  ...
  <div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
      <div>
        ...
        <p class="text-gray-600 mt-1">
          Gestiona segmentación y precios especiales por grupo
        </p>
      </div>
    ...
    <div v-if="stats" class="grid grid-cols-1 md:grid-cols-3 gap-4">
      ...
      <div class="card">
        ...
        <p class="text-2xl font-bold text-gray-900 mt-1">
          {{ groupStore.groups.filter((g) => g.is_active).length }}
        </p>
      </div>
      <div class="card">
        <h3 class="text-sm font-medium text-gray-500">
          Valor Promedio de Vida
        </h3>
        <p class="text-2xl font-bold text-gray-900 mt-1">
          L {{ formatNumber(stats.avg_lifetime_value) }}
        </p>
      </div>
    </div>
        ...
        <form @submit.prevent="handleSubmit" class="space-y-4">
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2"
              >Color</label
            >
            <input v-model="form.color" type="color" class="h-10 w-full" />
          </div>
    ...
    >
      ...
      >
        ...
        <div class="bg-gray-50 p-4 rounded-lg mb-4">
          ...
          <form @submit.prevent="handleAddPrice" class="space-y-3">
                ...
                >
                  ...
                  >
                    ...
                    <span class="text-xs text-gray-500"
                      >L {{ formatNumber(product.price) }}</span
                    >
                  </button>
                </div>
              </div>
            ...
            <div class="flex gap-2">
              <button
                type="submit"
                :disabled="!priceForm.product_id || groupStore.loading"
                class="btn-primary"
              >
        ...
        <!-- Prices List -->
        <div
          v-if="groupStore.prices.length === 0"
          ...
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="price in groupStore.prices" :key="price.id">
                ...
                <td class="px-4 py-2 text-sm text-gray-500">
                  L {{ formatNumber(price.product?.price) }}
                </td>
                <td class="px-4 py-2 text-sm font-medium text-green-600">
                  L {{ formatNumber(price.price) }}
                </td>
                <td class="px-4 py-2 text-sm text-gray-500">
                  <div v-if="price.valid_from || price.valid_until">
                    {{ formatDate(price.valid_from) || "Desde siempre" }} -
                    {{ formatDate(price.valid_until) || "Sin límite" }}
                  </div>
