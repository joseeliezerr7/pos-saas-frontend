    ...
    <div v-else class="space-y-4">
      ...
      >
        ...
        <p class="mt-1 text-sm text-gray-500">
          No hay aperturas registradas para esta caja
        </p>
      </div>

      <div
        v-for="opening in historyData"
        :key="opening.id"
          ...
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-xs text-gray-600 mt-1">
                Abierta por: {{ opening.user?.name }} el
                {{ formatDateTime(opening.opened_at) }}
              </p>
              <p v-if="opening.closing" class="text-xs text-gray-600">
                Cerrada por: {{ opening.closing.user?.name }} el
                {{ formatDateTime(opening.closing.closed_at) }}
              </p>
            </div>
            <div class="text-right">
              <p class="text-xs text-gray-600">Monto Inicial</p>
              <p class="text-lg font-bold text-gray-900">
                L {{ formatMoney(opening.opening_amount) }}
              </p>
            </div>
          </div>
        </div>
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              ...
              <p class="text-sm font-semibold text-gray-900">
                L {{ formatMoney(opening.stats.total_cash_sales) }}
              </p>
            </div>
            <div>
              <p class="text-xs text-gray-600">Ventas Tarjeta</p>
              <p class="text-sm font-semibold text-gray-900">
                L {{ formatMoney(opening.stats.total_card_sales) }}
              </p>
            </div>
            <div>
              <p class="text-xs text-gray-600">Total Ventas</p>
              <p class="text-sm font-semibold text-gray-900">
                L {{ formatMoney(opening.stats.total_sales) }}
              </p>
            </div>
            <div>
              <p class="text-xs text-gray-600">Esperado</p>
              <p class="text-sm font-semibold text-gray-900">
                L {{ formatMoney(opening.stats.expected_amount) }}
              </p>
            </div>
          </div>
        </div>
        ...
        >
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <p class="text-sm font-semibold text-gray-900">
                L {{ formatMoney(opening.closing.actual_amount) }}
              </p>
            </div>
            <div>
...
onMounted(async () => {
  ...
  try {
    selectedRegister.value =
      await cashRegisterStore.fetchCashRegister(cashRegisterId);
  } catch (error) {
