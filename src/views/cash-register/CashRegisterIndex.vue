  ...
  <div class="p-6">
    <div class="flex justify-between items-center mb-6">
      ...
      <div class="flex items-center space-x-3">
        <button
          @click="$router.push({ name: 'cash-register-reports' })"
        ...
        <button
          v-if="
            (!currentOpening || !currentOpening.is_open) &&
        ...
        <button
          v-else-if="currentOpening?.is_open && can('close_cash_register')"
    ...
    >
      <div class="flex items-center justify-between mb-4">
        <div>
          ...
          <p class="text-sm text-green-700">
            ...
            <span class="font-semibold">{{
              currentOpening.user?.name || "Usuario"
            }}</span>
          </p>
          <p class="text-sm text-green-700">
        ...
        <div class="text-right">
          ...
          <p class="text-2xl font-bold text-green-900">
            L {{ formatMoney(currentOpening.opening_amount) }}
          </p>
        </div>
      </div>
      ...
      <div
        v-if="summary"
        class="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4 pt-4 border-t border-green-200"
      >
        <div class="text-center">
          <p class="text-sm text-green-700">Ventas Efectivo</p>
          <p class="text-xl font-bold text-green-900">
            L {{ formatMoney(summary.total_cash_sales) }}
          </p>
        </div>
        <div class="text-center">
          <p class="text-sm text-green-700">Ventas Tarjeta</p>
          <p class="text-xl font-bold text-green-900">
            L {{ formatMoney(summary.total_card_sales) }}
          </p>
        </div>
        <div class="text-center">
          <p class="text-sm text-green-700">Total Ventas</p>
          <p class="text-xl font-bold text-green-900">
            L {{ formatMoney(summary.total_sales) }}
          </p>
        </div>
        <div class="text-center">
          <p class="text-sm text-green-700">Esperado en Caja</p>
          <p class="text-xl font-bold text-green-900">
            L {{ formatMoney(summary.expected_amount) }}
          </p>
        </div>
      </div>
          ...
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-sm text-gray-500">
                Sucursal: {{ register.branch?.name || "N/A" }}
              </p>
            </div>
            <div class="flex items-center space-x-3">
              <span
                v-if="currentOpening && selectedRegister?.id === register.id"
              ...
              <button
                v-if="
                  (!currentOpening || selectedRegister?.id !== register.id) &&
                  can('open_cash_register')
                "
            ...
            <form @submit.prevent="closeCashRegister">
              ...
              <div class="mb-4 p-3 bg-gray-50 rounded-lg">
                ...
                <p class="text-2xl font-bold text-gray-900">
                  L {{ formatMoney(summary?.expected_amount || 0) }}
                </p>
              </div>
                ...
                <div v-if="showDenominations" class="space-y-4">
                  ...
                  <div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                      ...
                      >
                        <label class="text-xs text-gray-600 w-16"
                          >L {{ denom.value }}</label
                        >
                        <input
                          v-model.number="
                            closeForm.denominations.bills[denom.key]
                          "
                          type="number"
                          min="0"
                        ...
                        <span class="text-xs text-gray-500 w-20 text-right">
                          ...
                          {{
                            formatMoney(
                              denom.value *
                                (closeForm.denominations.bills[denom.key] || 0),
                            )
                          }}
                        </span>
                      </div>
                  ...
                  <div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                      <div
                        v-for="denom in coinDenominations"
                        :key="denom.value"
                        class="flex items-center space-x-2"
                      >
                        <label class="text-xs text-gray-600 w-16"
                        ...
                        <input
                          v-model.number="
                            closeForm.denominations.coins[denom.key]
                          "
                          type="number"
                          min="0"
                          @input="calculateFromDenominations"
                        ...
                        <span class="text-xs text-gray-500 w-20 text-right">
                          ...
                          {{
                            formatMoney(
                              denom.value *
                                (closeForm.denominations.coins[denom.key] || 0),
                            )
                          }}
                        </span>
                      </div>
                  ...
                  <div class="pt-3 border-t border-gray-200">
                    <div class="flex justify-between items-center">
                      <span class="text-sm font-semibold text-gray-700"
                        >Total del Desglose:</span
                      >
                      <span class="text-lg font-bold text-primary-600"
                        >L {{ formatMoney(denominationTotal) }}</span
                      >
                    </div>
                    <p
                      v-if="
                        denominationTotal > 0 &&
                        closeForm.actual_amount &&
                        Math.abs(
                          denominationTotal -
                            parseFloat(closeForm.actual_amount),
                        ) > 0.01
                      "
              ...
              <div class="mb-4">
                ...
                <p class="text-xs text-gray-500 mt-1">
                  Puedes ingresar el monto manualmente o usar el desglose de
                  denominaciones arriba
                </p>
              </div>
              ...
              >
                ...
                <p
                  ...
                  :class="difference === 0 ? 'text-green-900' : 'text-red-900'"
                >
...
const difference = computed(() => {
  ...
  return (
    parseFloat(closeForm.value.actual_amount) -
    parseFloat(summary.value.expected_amount || 0)
  );
});
      ...
      for (const register of cashRegisters.value) {
        try {
          ...
          if (
            ...
            selectedRegister.value = register;
            console.log(
              "Found active opening for register:",
              register.name,
              cashRegisterStore.currentOpening,
            );
...
const addTransaction = async () => {
  try {
    ...
    await cashRegisterStore.addTransaction(
      selectedRegister.value.id,
