  ...
  <div class="payments-page">
    ...
    <div class="flex justify-between items-center mb-6">
      <div>
        ...
        <p class="text-gray-600 mt-1">Registro y seguimiento de pagos a proveedores</p>
      </div>
      <button
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="payment in payments" :key="payment.id" class="hover:bg-gray-50">
            ...
            <td class="px-6 py-4 whitespace-nowrap">
              ...
              <div class="text-sm text-gray-500">{{ payment.supplier?.rtn || 'N/A' }}</div>
            </td>
            <td class="px-6 py-4 whitespace-nowrap">
        ...
        <div class="p-6">
          ...
          <div class="grid grid-cols-2 gap-4 mb-6">
            <div>
              ...
              <p class="text-gray-900 font-medium">{{ selectedPayment?.supplier?.name }}</p>
            </div>
            <div>
          ...
          <div v-if="selectedPayment?.allocations?.length > 0">
            ...
            <div class="overflow-x-auto">
              <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                  <tr>
                    <th class="px-4 py-2 text-left text-xs font-medium text-gray-500">No. Compra</th>
                    <th class="px-4 py-2 text-right text-xs font-medium text-gray-500">Total Compra</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                  <tr v-for="allocation in selectedPayment.allocations" :key="allocation.id">
                    <td class="px-4 py-2 text-sm text-gray-900">{{ allocation.supplier_payable?.purchase?.purchase_number }}</td>
                    <td class="px-4 py-2 text-sm text-right">L {{ formatCurrency(allocation.supplier_payable?.original_amount) }}</td>
                    <td class="px-4 py-2 text-sm text-right font-semibold text-green-600">L {{ formatCurrency(allocation.amount_allocated) }}</td>
                    <td class="px-4 py-2 text-sm text-right">L {{ formatCurrency(allocation.supplier_payable?.balance_due) }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
    ...
    <div v-if="showPaymentModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" @click.self="closePaymentModal">
      <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4">
        <div class="p-6">
          <h2 class="text-2xl font-bold text-gray-800 mb-6">Registrar Pago a Proveedor</h2>
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
              <div class="col-span-2">
                ...
                >
                  ...
                  <option v-for="supplier in suppliers" :key="supplier.id" :value="supplier.id">
                    {{ supplier.name }} - Saldo: L {{ formatCurrency(supplier.current_balance || 0) }}
                  </option>
                </select>
              </div>
              ...
              <div v-if="['card', 'transfer', 'qr', 'check'].includes(paymentForm.payment_method)">
...
const payments = computed(() => payableStore.payments || [])
...
const loadPayments = async () => {
  loading.value = true
  try {
    await payableStore.fetchPayments(filters.value)
  } catch (error) {
...
const submitPayment = async () => {
  ...
  try {
    await payableStore.createPayment(paymentForm.value)
...
const viewPaymentDetails = async (payment) => {
  try {
    const details = await payableStore.fetchPaymentById(payment.id)
...
const getReferencePlaceholder = (method) => {
  const placeholders = {
    ...
    check: 'Ej: CHQ-001234, número de cheque',
...
const getReferenceHelpText = (method) => {
  const helpTexts = {
    ...
    check: 'Ingresa el número de cheque para seguimiento',
