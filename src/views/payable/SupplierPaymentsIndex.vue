    ...
    >
      ...
      >
        <div class="p-6">
          ...
          <!-- Amount Box -->
          <div
            class="bg-blue-50 border-2 border-blue-200 rounded-lg p-4 mb-6 text-center"
          >
            <div class="text-sm text-gray-600 mb-1">Monto Pagado</div>
            <div class="text-3xl font-bold text-blue-600">
              L {{ formatCurrency(selectedPayment?.amount) }}
            </div>
          </div>
          ...
          <div class="grid grid-cols-2 gap-4 mb-6">
            <div class="bg-gray-50 p-4 rounded-lg">
              ...
              <div class="text-xl font-semibold text-gray-900">
                L {{ formatCurrency(selectedPayment?.balance_before) }}
              </div>
            </div>
            <div class="bg-green-50 p-4 rounded-lg">
              <div class="text-sm text-gray-600 mb-1">Nuevo Balance</div>
              <div class="text-xl font-semibold text-green-600">
                L {{ formatCurrency(selectedPayment?.balance_after) }}
              </div>
            </div>
          </div>
          ...
          <div v-if="selectedPayment?.allocations?.length > 0">
            ...
            <div class="overflow-x-auto">
              <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                  <tr
                    v-for="allocation in selectedPayment.allocations"
                    :key="allocation.id"
                  >
                    <td class="px-4 py-2 text-sm text-gray-900">
                      {{
                        allocation.supplier_payable?.purchase?.purchase_number
                      }}
                    </td>
                    <td class="px-4 py-2 text-sm text-right">
                      ...
                      {{
                        formatCurrency(
                          allocation.supplier_payable?.original_amount,
                        )
                      }}
                    </td>
                    <td
                    ...
                    <td class="px-4 py-2 text-sm text-right">
                      L
                      {{
                        formatCurrency(allocation.supplier_payable?.balance_due)
                      }}
                    </td>
                  </tr>
                </tbody>
        ...
        <div class="p-6">
          ...
          <form @submit.prevent="submitPayment">
            <div class="grid grid-cols-2 gap-4 mb-4">
              ...
              <div
                v-if="
                  ['card', 'transfer', 'qr', 'check'].includes(
              ...
              >
                ...
                <input
                  ...
                  class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                  :placeholder="
                    getReferencePlaceholder(paymentForm.payment_method)
                  "
                />
...
const submitPayment = async () => {
  ...
  try {
    ...
    toast.error(
      error.response?.data?.error?.message || "Error al registrar el pago",
    );
  } finally {
