<template>
  <div class="space-y-6">
    ...
    <template v-else-if="payableStore.dashboardStats">
          ...
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-2xl font-bold text-gray-900">
                L {{ formatNumber(payableStore.dashboardStats.total_payable) }}
              </p>
              <p class="text-xs text-gray-500 mt-1">
                {{ payableStore.dashboardStats.total_payables }} facturas
              </p>
            </div>
          ...
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-2xl font-bold text-red-600">
                L {{ formatNumber(payableStore.dashboardStats.overdue_amount) }}
              </p>
              <p class="text-xs text-gray-500 mt-1">
                {{ payableStore.dashboardStats.overdue_count }} facturas
              </p>
            </div>
          ...
          <div class="flex items-center justify-between">
            <div class="flex-1">
              ...
              <div class="mt-1">
                <p
                  v-if="
                    !payableStore.dashboardStats.suppliers_over_limit?.count ||
                    payableStore.dashboardStats.suppliers_over_limit.count === 0
                  "
          ...
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-2xl font-bold text-green-600">
                L
                {{
                  formatNumber(payableStore.dashboardStats.payments_this_month)
                }}
              </p>
              <p class="text-xs text-gray-500 mt-1">
                {{ payableStore.dashboardStats.payment_count_this_month }} pagos
              </p>
            </div>
      ...
      <div class="card">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div
            v-if="payableStore.dashboardStats.aging"
          ...
          >
            ...
            <p class="text-2xl font-bold text-green-900 mt-1">
              ...
              {{
                formatNumber(
                  payableStore.dashboardStats.aging.current?.amount || 0,
                )
              }}
            </p>
            <p class="text-xs text-green-600 mt-1">
              {{ payableStore.dashboardStats.aging.current?.count || 0 }}
              facturas
            </p>
          </div>
          <div
          ...
          >
            ...
            <p class="text-2xl font-bold text-yellow-900 mt-1">
              ...
              {{
                formatNumber(
                  payableStore.dashboardStats.aging["1_30_days"]?.amount || 0,
                )
              }}
            </p>
            <p class="text-xs text-yellow-600 mt-1">
              {{ payableStore.dashboardStats.aging["1_30_days"]?.count || 0 }}
              facturas
            </p>
          </div>
          <div
          ...
          >
            ...
            <p class="text-2xl font-bold text-orange-900 mt-1">
              ...
              {{
                formatNumber(
                  payableStore.dashboardStats.aging["31_60_days"]?.amount || 0,
                )
              }}
            </p>
            <p class="text-xs text-orange-600 mt-1">
              {{ payableStore.dashboardStats.aging["31_60_days"]?.count || 0 }}
              facturas
            </p>
          </div>
          <div
          ...
          >
            ...
            <p class="text-2xl font-bold text-red-900 mt-1">
              ...
              {{
                formatNumber(
                  payableStore.dashboardStats.aging["60_plus_days"]?.amount ||
                    0,
                )
              }}
            </p>
            <p class="text-xs text-red-600 mt-1">
              {{
                payableStore.dashboardStats.aging["60_plus_days"]?.count || 0
              }}
              facturas
            </p>
          </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="card">
          <h2 class="text-lg font-semibold mb-4">
            Principales Proveedores con Saldo
          </h2>
          <div
            v-if="
              !payableStore.dashboardStats.top_suppliers ||
              payableStore.dashboardStats.top_suppliers.length === 0
            "
          ...
          <div v-else class="space-y-3">
            <div
              v-for="(
                supplier, index
              ) in payableStore.dashboardStats.top_suppliers.slice(0, 5)"
              :key="supplier.id"
              ...
              <div class="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p class="text-sm text-gray-500">
                    {{ supplier.pending_payables }} facturas pendientes
                  </p>
                </div>
              </div>
              <div class="text-right">
                <p class="font-semibold text-gray-900">
                  L {{ formatNumber(supplier.balance) }}
                </p>
              </div>
            </div>
          </div>
        ...
        <div class="card">
          ...
          <div
            v-if="
              !payableStore.dashboardStats.recent_payments ||
              payableStore.dashboardStats.recent_payments.length === 0
            "
            class="text-center py-8 text-gray-500"
          >
          ...
          <div v-else class="space-y-3">
            <div
              v-for="payment in payableStore.dashboardStats.recent_payments.slice(
                0,
                5,
              )"
              :key="payment.id"
    ...
    >
      <div
        class="bg-white rounded-lg shadow-xl max-w-3xl w-full mx-4 max-h-[80vh] overflow-hidden"
      >
        <div class="p-6 border-b border-gray-200">
          <div class="flex justify-between items-center">
            <div>
              <h2 class="text-2xl font-bold text-gray-800">
                Proveedores sobre Límite de Crédito
              </h2>
              <p class="text-sm text-gray-600 mt-1">
                {{
                  payableStore.dashboardStats?.suppliers_over_limit?.count || 0
                }}
                proveedores requieren atención
              </p>
            </div>
            <button
        ...
        <div class="p-6 overflow-y-auto max-h-[60vh]">
          <div
            v-if="
              payableStore.dashboardStats?.suppliers_over_limit?.suppliers
                ?.length > 0
            "
            class="space-y-4"
          >
            <div
              v-for="supplier in payableStore.dashboardStats
                .suppliers_over_limit.suppliers"
              :key="supplier.id"
                  ...
                  <div class="grid grid-cols-2 gap-4 mt-3">
                    <div>
                      ...
                      <p class="text-sm font-medium text-gray-900">
                        L {{ formatNumber(supplier.credit_limit) }}
                      </p>
                    </div>
                    <div>
                      <p class="text-xs text-gray-500">Balance Actual</p>
                      <p class="text-sm font-semibold text-orange-600">
                        L {{ formatNumber(supplier.current_balance) }}
                      </p>
                    </div>
                    <div>
          ...
          <div v-else class="text-center py-12">
            ...
            <p class="text-gray-600 font-medium">
              Todos los proveedores dentro del límite de crédito
            </p>
...
function showSuppliersOverLimit() {
  if (
    payableStore.dashboardStats?.suppliers_over_limit?.suppliers?.length > 0
  ) {
