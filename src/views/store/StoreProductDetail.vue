  ...
  <div>
    ...
    <nav class="mb-6 text-sm">
      <ol class="flex items-center gap-2 text-gray-500">
        <li>
          <router-link :to="`/store/${storeSlug}`" class="hover:text-blue-600"
            >Inicio</router-link
          >
        </li>
        <li>/</li>
        <li v-if="product?.category">
          <router-link
            :to="`/store/${storeSlug}?category=${product.category.id}`"
    ...
    <div v-else class="grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      <div class="space-y-6">
        <div>
          <p v-if="product.category" class="text-sm text-blue-600 mb-2">
            {{ product.category.name }}
          </p>
        ...
        <div
          v-if="product.ecommerce_description || product.description"
...
const loadProduct = async () => {
  loading.value = true;
  try {
    const response = await publicStoreService.getProduct(
      props.storeSlug,
      route.params.productId,
    );
