<template>
  <div>
    ...
    <div
      ...
      class="mb-8 bg-gradient-to-r from-blue-600 to-purple-600 rounded-2xl p-8 text-white"
    >
      <h1 class="text-3xl font-bold mb-2">
        Bienvenido a {{ store?.store_name }}
      </h1>
      <p class="text-blue-100">
        {{ store?.description || "Encuentra los mejores productos aqui" }}
      </p>
    </div>
    ...
    <div v-else class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
      ...
      >
        ...
        <div class="p-4">
          <router-link :to="`/store/${storeSlug}/product/${product.id}`">
            <h3
              class="font-medium text-gray-900 mb-1 line-clamp-2 hover:text-blue-600"
            >
              {{ product.name }}
            </h3>
          </router-link>
...
const loadProducts = async (page = 1) => {
  ...
  try {
    ...
    const response = await publicStoreService.getProducts(
      props.storeSlug,
      params,
    );
