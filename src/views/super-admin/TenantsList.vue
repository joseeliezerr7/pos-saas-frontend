<template>
  <div class="tenants-list">
    ...
    <div v-else class="bg-white rounded-lg shadow overflow-hidden">
            ...
            >
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <span
                  ...
                  :class="getPlanClass(tenant.subscription.plan.slug)"
                >
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                ...
                <div
                  v-if="tenant.subscription?.expires_at"
      ...
      <!-- Paginación -->
      <div
        v-if="tenants.data && tenants.data.length > 0"
        class="bg-white px-4 py-3 border-t border-gray-200 sm:px-6"
      >
        <div class="flex items-center justify-between">
          <div class="text-sm text-gray-700">
            Mostrando <span class="font-medium">{{ tenants.from }}</span> a
            <span class="font-medium">{{ tenants.to }}</span> de
            <span class="font-medium">{{ tenants.total }}</span> clientes
          </div>
          <div class="flex space-x-2">
            <button
              ...
              class="btn-secondary"
              :class="{
                'opacity-50 cursor-not-allowed': !tenants.prev_page_url,
              }"
            >
              Anterior
            </button>
            <button
              ...
              class="btn-secondary"
              :class="{
                'opacity-50 cursor-not-allowed': !tenants.next_page_url,
              }"
            >
              Siguiente
            </button>
...
async function toggleStatus(tenant) {
  ...
  try {
    ...
    toast.success(
      `Cliente ${action === "desactivar" ? "desactivado" : "activado"} exitosamente`,
    );
    await loadTenants();
  } catch (error) {
...
async function confirmDelete(tenant) {
  if (
    !confirm(
      `¿Estás seguro de eliminar el cliente "${tenant.name}"?\n\nEsta acción no se puede deshacer.`,
    )
  ) {
    return;
  }
