  ...
  <div class="plans-list">
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <div
        ...
        :class="
          plan.is_active ? 'border-transparent' : 'border-red-200 opacity-75'
        "
      >
        <!-- Plan Header -->
        ...
        >
          ...
          <div class="flex items-center space-x-3">
            ...
            <button
              ...
              :class="{
                'opacity-50 cursor-not-allowed': plan.active_subscriptions > 0,
              }"
            >
              <svg
                class="w-5 h-5"
            ...
            <div class="mt-4">
              <label class="flex items-center">
                ...
                <span class="ml-2 text-sm text-gray-700"
                  >Plan activo (visible para nuevas suscripciones)</span
                >
              </label>
            </div>
...
async function savePlan() {
  ...
  try {
    ...
    const message =
      error.response?.data?.error?.message || "Error al guardar el plan";
    toast.error(message);
  } finally {
    saving.value = false;
  }
...
async function toggleStatus(plan) {
  ...
  try {
    ...
    toast.success(
      `Plan ${action === "desactivar" ? "desactivado" : "activado"} exitosamente`,
    );
    await loadPlans();
  } catch (error) {
...
async function confirmDelete(plan) {
  ...
  try {
    ...
    const message =
      error.response?.data?.error?.message || "Error al eliminar el plan";
    toast.error(message);
  }
}
