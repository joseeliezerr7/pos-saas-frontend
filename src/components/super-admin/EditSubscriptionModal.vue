        ...
        <div>
          ...
          <select v-model="form.plan_id" required class="input">
            ...
            <option v-for="plan in plans" :key="plan.id" :value="plan.id">
              ...
              {{
                formatCurrency(
                  plan.price_monthly || plan.monthly_price || plan.price || 0,
                )
              }}/mes
            </option>
          </select>
        ...
        <div v-if="selectedPlan" class="bg-gray-50 rounded-lg p-4">
          ...
          <div class="grid grid-cols-2 gap-3 text-sm">
            <div class="flex items-center">
              ...
              <span>{{
                selectedPlan.max_users === -1
                  ? "Usuarios ilimitados"
                  : `${selectedPlan.max_users} usuarios`
              }}</span>
            </div>
            <div class="flex items-center">
              <svg
              ...
              <span>{{
                selectedPlan.max_branches === -1
                  ? "Sucursales ilimitadas"
                  : `${selectedPlan.max_branches} sucursales`
              }}</span>
            </div>
            <div class="flex items-center">
              <svg
              ...
              <span>{{
                selectedPlan.max_products === -1
                  ? "Productos ilimitados"
                  : `${selectedPlan.max_products} productos`
              }}</span>
            </div>
            <div class="flex items-center">
              <svg
              ...
              <span>{{
                selectedPlan.max_monthly_transactions === -1
                  ? "Transacciones ilimitadas"
                  : `${selectedPlan.max_monthly_transactions} transacciones/mes`
              }}</span>
            </div>
          </div>
        </div>
            ...
            <div>
              ...
              <ul class="text-sm text-yellow-800 space-y-1">
                ...
                <li v-if="statusChanged">
                  ...
                  → <strong>{{ getSubscriptionLabel(form.status) }}</strong>
                </li>
                <li v-if="expiresAtChanged">
                  ...
                  →
                  <strong>{{
                    formatDate(form.expires_at) || "En 30 días"
                  }}</strong>
                </li>
              </ul>
            </div>
  ...
  if (props.tenant.subscription) {
    ...
    if (props.tenant.subscription.expires_at) {
      form.value.expires_at =
        props.tenant.subscription.expires_at.split("T")[0];
    }
  }
});
...
async function handleSubmit() {
  ...
  try {
    await superAdminService.updateTenantSubscription(
      props.tenant.id,
      form.value,
    );
